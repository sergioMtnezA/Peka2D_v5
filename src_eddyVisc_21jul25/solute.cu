...
/*----------------------------*/
    ...
    double sigmap, sigmah, sigmaD;
    double ra, As, Ai, Asigma, n;
    double uestrelia;
    ...
    if(i<nTasks){
        ...
        if(arrays->h[idx] >= arrays->minh){
            aux1 = 0.0;
            uestrelia = _g_*sqrt(nman2wall*nman2wall*moduloU*moduloU/powf(arrays->h[idx],1./3.));

            for(jphi=0;jphi<nInterfaces;jphi++){

                #if SET_EDDYVISCOSITY
                sigmap = 1-0.3;
                sigmah = 0.4;
                ra = 0.99;
                n = 1.;
                ...
                sigmaD = (jphi + hlayer*0.5)/arrays->h[idx];
                As = ra*(1. + sigmap)*(1. + sigmap)/4.;
                Ai = (sigmah + 1)*(sigmap-sigmah);

                if((sigmaD>0)&&(sigmaD<sigmah)){
                    Asigma = (sigmaD+1)*(sigmap-sigmaD);
                }else if ((sigmaD<1) && (sigmaD>sigmah)){
                    Asigma = (Ai - As)*powf(sigmaD/sigmah, n) + As;
                }
