...
int read_peka_mesh(t_mesh *mesh, char *filename){
	...
	for(i=0;i<mesh->ncells;i++){
		fscanf(fp,"%d %d %d",&mesh->cell[i].n1,&mesh->cell[i].n2,&mesh->cell[i].n3); // Los id de los nodos empiezan en 1
	}
...
int write_fed_mesh(t_mesh *mesh, char *filename){
	...
	fprintf(fp,"%8d %8d    0    0\n",mesh->ncells,mesh->nnodes);


	for(i=0; i<mesh->nnodes; i++){
		fprintf(fp,"%8d %12.6lf %12.6lf    0.000    0.000    -9999.000    0    0\n",mesh->node[i].id,mesh->node[i].x,mesh->node[i].y);
	}

	printf("%s Nodes written %d\n",OK,mesh->nnodes);

	for(i=0; i<mesh->ncells; i++){
  		fprintf(fp,"%8d %8d %8d %8d %3.3lf %12.6lf %12.6lf %12.6lf    0.000\n",mesh->cell[i].id,mesh->cell[i].n1,mesh->cell[i].n2,mesh->cell[i].n3,mesh->cell[i].nMan,mesh->cell[i].z,mesh->cell[i].wsl,mesh->cell[i].zR);

		//mesh->cell[i].cx=(n1->x+n2->x+n3->x)/3.;
		//mesh->cell[i].cy=(n1->y+n2->y+n3->y)/3.;
		//mesh->cell[i].area=triangle_size(n1,n2,n3);
	}
...
int main (int argc, char * argv[]){
	...
	//sprintf(filename,"%s_conf.vtk",argv[1]);
