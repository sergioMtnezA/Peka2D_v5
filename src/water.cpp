...
/*----------------------------*/
    ...
    int i;
	#if SET_OPENMP
    omp_set_num_threads(arrays->ncores);
    #pragma omp parallel for \
        default(none) \
        shared(nTasks,arrays) \
        private(idx) \
        reduction(+:auxMass)
    #endif
    for(i=0;i<nTasks;i++){
        ...
        arrays->mass[idx] = arrays->h[idx] * arrays->area[idx];
        auxMass+=arrays->mass[idx];
    }

	arrays->massNew=auxMass;
}
...
/*----------------------------*/
    ...
    #pragma omp parallel for \
        ...
        private(idx, \
            ...
            modUL, modUR, \
            unL, unR, vtL, vtR, \
            hbar, unbar,vtbar,cbar,inverseCbar, \
            nx,ny, \
            deltah,deltahu,deltahv, \
            aux1,aux2,aux3,aux4, \
            gp, \
            deltaXl, \
            sqrghL,sqrghR, \
            ...
            beta_z, \
            dZ,dL,dR,dZs, \
            bedSlope_term,ps1,ps2,psmax, \
            hqi0,hqi1,hqi2, \
            beta_f, \
            maxh, minU, manning2, \
            modUbar, nux, nuy, dint, tau, stress, auxTau, \
            friction_term, friction_t, \
            hstar,hls,hrs, \
            eigel,landl,landEl,alfal,betal, \
            dUL,dUR,dULrot,dURrot, \
            solidWall,wetWall, \
            qnormalL, \
            dtl )
    #endif        
    for(i=0;i<nTasks;i++){
        ...
        //ccccccccccccccccccccccccccccccccccccccccccccccc update active cell list
 		if(wetWall==0){
            ...
            {
			#endif
            c_add_active_cells(arrays, id1);
            c_add_active_cells(arrays, id2);           
			//	printf("Pair: %d %d\n",c1->id, c2->id);
			#if SET_OPENMP
			}
			#endif
...
/*----------------------------*/
    ...
    #endif
    for(i=0;i<nTasks;i++){	
		minDt=fmin(minDt, arrays->localDt[i]);
    }
...
/*----------------------------*/
    ...
    for(i=0;i<nTasks;i++){  
        ...
        if((h1 > TOL12) || (h2 > TOL12)){
			...
			{
			#endif
            pos = arrays->nActWalls;
            arrays->actWalls[pos] = idxw;
            arrays->nActWalls++;
...
/*----------------------------*/
    int access;
    access=arrays->activeC[id];
    if(access==0){
        arrays->activeC[id]=1;
        arrays->actCells[arrays->nActCells]=id;
...
/*----------------------------*/
    ...
    for(k=0;k<arrays->NCwall;k++){
        if(arrays->neighCell[k*arrays->ncells+id] >= 0){ //internal wall
            ...
            access=arrays->activeW[idxw];
            if(access==0){
                arrays->activeW[idxw]=1;
                arrays->actWalls[arrays->nActWalls]=idxw;
                //printf("Cell %d :: Wall %d added to active wall position %d\n",id,idxw,arrays->nActWalls);
                arrays->nActWalls++;
            }
        }
    }
